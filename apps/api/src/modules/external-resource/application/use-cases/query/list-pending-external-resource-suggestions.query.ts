...
@Injectable()
export class ListPendingExternalResourceSuggestionsQuery {
  constructor(private readonly repository: ExternalResourceSuggestionRepository) {}
  execute(query: QueryResourceDTO): Promise<Paginated<ExternalResourceSuggestionDTO>> {
    return this.repository.listPending(query).then((result) => ({
      ...result,
