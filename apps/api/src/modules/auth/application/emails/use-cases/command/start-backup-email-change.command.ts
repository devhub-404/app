...
import { JwtTokenType, EmailChangeBackupTokenDTO } from '@/modules/auth/application/shared/dto';
import {
...
@Injectable()
export class StartBackupEmailChangeCommand {
  constructor(
  ...
  async execute(userId: string, sessionId: string, email: string): Promise<GenericAuthenticatedAckDTO> {
    ...
    const token = await this.flowTokenService.signSingleUse({
      ...
      purpose: 'email_change_backup',
      payload: { sub: userId, email } satisfies Partial<EmailChangeBackupTokenDTO>,
      expiresIn: '15m',
    });
