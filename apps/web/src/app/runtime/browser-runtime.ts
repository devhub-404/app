...
import { initializeSessionScope } from '@/shared/runtime/session-scope';

let started = false;

type StaticSearchSection = HTMLElement & { dataset: DOMStringMap & { ready?: string } };
...
function syncPersistentShellNavigation() {
  ...
  for (const link of document.querySelectorAll<HTMLElement>('[data-shell-nav-link]')) {
...
function initStaticSearch(section: StaticSearchSection) {
  ...
  const query = section.querySelector<HTMLInputElement>('[data-static-content-query]');
  const clear = section.querySelector<HTMLButtonElement>('[data-static-content-clear]');
  const result = section.querySelector<HTMLElement>('[data-static-content-result]');
...
function initPageRuntime() {
  ...
  for (const section of document.querySelectorAll<StaticSearchSection>('[data-static-content-search]')) {
