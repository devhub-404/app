...
export function useResourceInteractions(items: Accessor<ResourceItem[]>) {
  ...
  createEffect(() => {
    ...
    for (const item of currentItems) {
      if (nextVoteCounts[item.id] === undefined) nextVoteCounts[item.id] = item.votes;
    }
    setState({ voteCounts: nextVoteCounts, personal: {}, personalLoading: authenticated && ids.length > 0 });
  ...
  const vote = async (item: ResourceItem) => {
    if (state.pending[item.id] || state.personalLoading || !requireAccount()) return;
  ...
  const votesFor = (item: ResourceItem) => state.voteCounts[item.id] ?? item.votes;
