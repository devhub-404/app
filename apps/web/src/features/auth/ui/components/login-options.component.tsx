...
import { preloadPasskeyAuthentication, preloadPasswordAuthentication } from '@/features/auth/utils/browser-auth.util.ts';
...
function LoginOptions(props: { locale: Locale }) {
  ...
  const [method, setMethod] = createSignal<'password' | 'magic-link'>('password');
  ...
  const returnTo = () => (typeof window === 'undefined' ? routes.feed : returnToFromSearch(window.location.search));

  const completeAuthentication = (redirect?: string | null, code?: string | null) => {
  ...
  const oauth = async (provider: OAuthProvider) => {
    ...
    const result = await startOAuth(provider, { redirect: returnTo(), flow: 'login' });
    if (result.error) {
      setOutcome({ kind: 'error', code: result.error.code, message: result.error.message });
      setPendingMethod(null);
      return;
    }
  ...
  return (
    <div class="space-y-5">
      ...
      <Show when={!outcome() && method() === 'password'}>
        <LoginForm locale={props.locale} onResult={acceptResult} ready={ready()} />
        ...
        <section class="space-y-3" aria-labelledby="login-alternatives-heading">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            <button
              ...
              onClick={() => void oauth('google')} aria-busy={pendingMethod() === 'google'} class="action action-secondary"
             
            >
              <Globe class="size-4" />
              {pendingMethod() === 'google' ? t('login.openingGoogle') : t('login.google')}
            </button>
            <button
              type="button"
              disabled={!ready() || pendingMethod() !== null}
             
              onClick={() => void oauth('github')} aria-busy={pendingMethod() === 'github'} class="action action-secondary"
             
            >
              <Github class="size-4" />
              {pendingMethod() === 'github' ? t('login.opening') : t('provider.github')}
            </button>
            <button
              type="button"
              disabled={!ready() || pendingMethod() !== null}
             
              onClick={() => void passkey()} aria-busy={pendingMethod() === 'passkey'} class="action action-secondary"
             
            >
              <KeyRound class="size-4" />
              {pendingMethod() === 'passkey' ? t('common.verifying') : t('login.passkey')}
            </button>
            <button type="button" disabled={!ready()} onClick={() => setMethod('magic-link')} class="action action-secondary">
              <Mail class="size-4" />
              {t('login.magicLink')}
            </button>
          </div>
      ...
      <Show when={!outcome() && method() === 'magic-link'}>
        <MagicLinkForm locale={props.locale} redirect={returnTo()} ready={ready()} />
      </Show>

      <Show when={outcome() || method() !== 'password'}>
        <Portal
          mount={
            typeof document === 'undefined' ? undefined : (document.getElementById('auth-top-action') ?? undefined)
          }
        >
