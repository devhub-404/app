...
import { $personalStateScope, getPersonalContentState, refreshPersonalState } from '@/shared/runtime/personal-state';
...
function ArticleCardActions(props: Props) {
  ...
  const share = async () => {
    const url = new URL(`/articles/${encodeURIComponent(props.slug)}`, window.location.origin).href;
  ...
  return (
    <span class="inline-flex items-center gap-[.45rem]">
      ...
      <button type="button" onClick={() => void share()} aria-label={t('articledetail.shareArticle')} class="action action-ghost">
      ...
      <Toggle.Root
        ...
        class="inline-flex cursor-pointer items-center border-0 bg-transparent p-0 hover:text-content-accent disabled:opacity-50 aria-pressed:text-content-accent"
      >
        <Bookmark class={`size-4 ${state.bookmarked ? 'fill-current' : ''}`} aria-hidden="true" />
      </Toggle.Root>
