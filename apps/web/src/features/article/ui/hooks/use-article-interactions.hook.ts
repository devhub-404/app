...
import { $personalStateScope, getPersonalContentStates, refreshPersonalState } from '@/shared/runtime/personal-state';
...
export function useArticleInteractions(items: Accessor<ArticleItem[]>) {
  ...
  createEffect(() => {
    ...
    setState('voteCounts', (current) => {
      const next = { ...current };
      for (const item of currentItems) if (next[item.id] === undefined) next[item.id] = item.votes;
      return next;
    });
  ...
  const requireAccount = () => {
    ...
    redirectTo(`/login?redirect=${encodeURIComponent(window.location.pathname + window.location.search)}`);
    return false;
  };

  const vote = async (item: ArticleItem) => {
    if (state.pending[item.id] || state.loading || !requireAccount()) return;
    const previous = state.personal[item.id] ?? { voted: false, bookmarked: false };
    ...
    try {
      ...
      setState('voteCounts', item.id, Math.max(0, (state.voteCounts[item.id] ?? item.votes) + (next ? 1 : -1)));
  ...
  const bookmark = async (item: ArticleItem) => {
    ...
    setState('messages', item.id, '');
    try {
      const result = next ? await saveBookmark(item.id) : await removeBookmark(item.id);
      if (!result || result.error) throw new Error('mutation failed');
  ...
  const share = async (item: ArticleItem) => {
    try {
      const url = new URL(`/articles/${encodeURIComponent(item.slug)}`, window.location.origin).href;
      const result = await shareOrCopy(item.title, url);
      if (result === 'copied') setState('messages', item.id, t('articlecard.linkCopied'));
      if (result === 'failed') setState('messages', item.id, t('articledetail.couldNotShare'));
    } catch {
      setState('messages', item.id, t('articledetail.couldNotShare'));
    }
  };

  const votesFor = (item: ArticleItem) => state.voteCounts[item.id] ?? item.votes;
  return {
