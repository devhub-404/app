...
export default function ProjectActions(props: Props) {
  ...
  const canEdit = createMemo(() => authenticated() && canEditProject(props.project, actor()));
  ...
  return (
    <>
      ...
      />
      <ReportButton target="resource" id={props.project.id} locale={props.locale} />
      <Show when={canEdit()}>
        <a
         
          href={routes.account.project(props.project.id)}
          title={props.manageLabel}
          aria-label={props.manageLabel} class="action action-ghost"
        >
