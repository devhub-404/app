...
import {
  createAccountLifecycleSchemas,
  type ReactivationCodeFormInput,
} from "@/features/account/ui/schemas/lifecycle.schema.ts";
...
function AccountReactivation(props: { token: string; locale: Locale }) {
  ...
  const [totpForm, { Form: TotpForm, Field: TotpField }] =
    createForm<ReactivationCodeFormInput>({
      validate: zodForm(schemas.totp),
    });
  const [recoveryForm, { Form: RecoveryForm, Field: RecoveryField }] =
    createForm<ReactivationCodeFormInput>({
      validate: zodForm(schemas.recovery),
    });
  const canUseTotp = createMemo(
    () => state.methods.length === 0 || state.methods.includes("totp"),
  );
  const canUseRecovery = createMemo(
    () => state.methods.length === 0 || state.methods.includes("recovery_code"),
  );
          ...
          <Show when={canUseTotp()}>
            <TotpForm onSubmit={finishTotp} class="grid gap-4">
              <TotpField name="code">
                {(field, fieldProps) => (
                  <Field>
                    <label for="reactivation-totp" class="field-label">
                      {t("lifecycle.reactivationTotp")}
                    </label>
                    <input
                      {...fieldProps}
              ...
              <button
                ...
                aria-busy={state.pending === "totp"}
                class="action action-primary"
              >
                <Check class="size-4" aria-hidden="true" />
                {state.pending === "totp"
                  ? t("lifecycle.reactivationLoading")
                  : t("lifecycle.reactivationConfirm")}
              </button>
            </TotpForm>
          </Show>
          <Show when={canUseRecovery()}>
            <RecoveryForm onSubmit={finishRecovery} class="grid gap-4">
              <RecoveryField name="code">
                {(field, fieldProps) => (
                  <Field>
                    <label for="reactivation-recovery" class="field-label">
                      {t("lifecycle.reactivationRecovery")}
                    </label>
                    <input
                      {...fieldProps}
              ...
              <button
                type="submit"
                disabled={state.pending !== null}
                aria-busy={state.pending === "recovery"}
