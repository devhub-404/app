...
import { createUpdateProfileSchema, type UpdateProfileFormInput } from '@/features/account/ui/schemas/profile/forms.schema.ts';
...
function ProfileForm() {
  ...
  const textFields = [
    ...
    ['headline', t('profileform.headline'), t('profileform.backendEngineerFocusedSystemsDistributed')],
  ...
  const avatarName = () => account().details?.profile.displayName ?? account().details?.profile.username ?? '';
  const avatarUrl = () => avatarPreview() ?? account().details?.profile.avatarUrl ?? null;
  const handleAvatarChange = (event: Event & { currentTarget: HTMLInputElement }) => {
  ...
  return (
    <Show when={!loading()} fallback={<p class="text-muted">{t('profileform.loading')}</p>}>
      <Form class="flex flex-col gap-4" onSubmit={save}>
        <Field>
          <label for="profile-avatar" class="field-label">{t('profileform.avatarOptionalUpTo1Mib')}</label>
          <div class="flex items-center gap-4">
            <div class="relative size-20 shrink-0">
              ...
              <button
                ...
                onClick={() => avatarInput?.click()} class="action action-ghost"
        ...
        <Field>
          <label for="profile-username" class="field-label">{t('profileform.usernameAlternative2')}</label>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <For each={textFields}>
            {([name, label, placeholder]) => (
              <Field>
                <label for={`profile-${name}`} class="field-label">{label}</label>
                <FormField name={name}>
                  {(field, fieldProps) => (
                    <>
                      <input
        ...
        <Field>
          <label for="profile-bio" class="field-label">{t('profileform.bio')}</label>
          <FormField name="bio">
            {(field, fieldProps) => (
              <textarea
                {...fieldProps}
        ...
        <p class="text-caption">{t('profileform.profileTagsDescribeTechnologiesNotSeniorityOrRole')}</p>
        <div class="flex flex-wrap gap-3">
          ...
          <Show when={currentUsername()}>
            <a href={routes.profile(currentUsername())} target="_blank" rel="noreferrer" class="action action-secondary">
              <ExternalLink class="size-4" /> {t('profileform.viewProfilePublic')}
            </a>
          </Show>
        </div>
