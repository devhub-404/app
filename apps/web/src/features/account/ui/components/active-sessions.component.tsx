...
import { refreshCurrentSession } from '@/features/auth/public/session';
...
function ActiveSessions() {
  ...
  const { refreshSessions, revokeSession, logoutAllSessions, logoutOtherSessions } = useAccount();
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const [result, currentSession] = await Promise.all([refreshSessions(), refreshCurrentSession()]);
      ...
      if (!currentSession) setError(t('activesessions.couldNotLoadSessionsNow'));
    } catch {
  ...
  const sortedSessions = createMemo(() =>
    [...sessions()].sort((a, b) => (b.lastProofOfPossessionAt ?? '').localeCompare(a.lastProofOfPossessionAt ?? '')),
  );
  ...
  const remove = async (id: string) => {
    ...
    try {
      ...
      if (result.kind === 'failure') setError(t('activesessions.couldNotCloseThisSession'));
    } catch {
  ...
  const logoutOthers = async () => {
    ...
    try {
      ...
      if (result.kind === 'failure') setError(t('activesessions.couldNotCloseOtherSessions'));
    } catch {
  ...
  return (
    <div class="space-y-5">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p class="text-muted">
            {t('activesessions.recognizeDevicesStillHasSessionValidAccessAccountCloseSession')}
          </p>
          <p class="text-caption mt-1">
            {t('activesessions.credentialSessionNeverShownThisScreenShowsOnlyMetadataNot')}
          </p>
        </div>
        <div class="flex shrink-0 flex-wrap gap-2">
          <button
            ...
            onClick={() => void logoutOthers()} aria-busy={pending() === 'others'} class="action action-secondary"
           
          >
            <LogOut class="size-4" aria-hidden="true" />
            {pending() === 'others' ? t('sessions.ending') : t('sessions.endOthers')}
          </button>
          <button
            type="button"
            disabled={pending() !== null}
           
            onClick={() => void logoutAll()} aria-busy={pending() === 'all'} class="action action-danger"
          ...
          <Show when={confirmAll()}>
            <button
              type="button"
              disabled={pending() !== null}
              onClick={() => setConfirmAll(false)} class="action action-secondary"
      ...
      <div class="rounded-3xl border border-line bg-surface-elevated p-4 shadow-sm" aria-live="polite">
        <Show when={!loading()} fallback={<p class="text-muted">{t('activesessions.loadingSessions')}</p>}>
          <Show
            when={sortedSessions().length > 0}
            fallback={
              <div class="space-y-3">
                <p class="text-muted">{t('activesessions.noSessionActiveFound')}</p>
                <button type="button" onClick={() => void load()} class="action action-secondary">
                    ...
                    <li class="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
                      <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                          ...
                          <Show when={current()}>
                            <StatusBadge status="accent" class="min-h-6 px-2 py-0.5 text-[11px]">
                              {t('activesessions.sessionCurrent')}
                            </StatusBadge>
                          </Show>
                        </div>
                        <p class="text-caption mt-1">
                          {session.authMethod} {t('activesessions.created')} {formatDate(session.createdAt, locale())}{' '}
                          {t('activesessions.lastProof')} {formatDate(session.lastProofOfPossessionAt, locale())}{' '}
                          {t('activesessions.expires')} {formatDate(session.expiresAt, locale())}
                        </p>
                      ...
                      <button
                        ...
                        onClick={() => void remove(session.id)} aria-busy={pending() === session.id} class="action action-danger-outline"
