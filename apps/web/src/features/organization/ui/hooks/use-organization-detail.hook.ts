...
import type { Organization, OrganizationMembership } from '@/features/organization/types/organization.type.ts';
...
import { getRouteScope, isCurrentRouteScope } from '@/shared/runtime/route-scope';
import { getSessionScope } from '@/shared/runtime/session-scope';

export type OrganizationContentItem = { id: string; title: string; slug?: string };

export function useOrganizationDetail(slug: () => string, initialOrganization?: Organization | null) {
  ...
  const loadRelated = async (organizationId: string) => {
    ...
    try {
      const jobs = await listJobsByOrganization(organizationId, 6, { signal: routeScope.signal });
  ...
  const load = async () => {
    ...
    try {
      const organizationResult = await getOrganizationQuery(slug(), { signal: routeScope.signal });
  ...
  const loadMembers = async (organizationId: string) => {
    ...
    try {
      ...
      const result = await listOrganizationMembersQuery(organizationId, {
        signal: combineAbortSignals(routeScope.signal, sessionScope.signal),
      });
      if (isCurrentRouteScope(routeScope)) setState('members', result.ok ? result.data : []);
    } catch {
