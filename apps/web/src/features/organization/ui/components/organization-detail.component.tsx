...
function OrganizationDetail(props: {
  ...
  const detail = useOrganizationDetail(
    () => props.slug,
  ...
  return (
    <Show
      ...
      fallback={
        <LoadingState>
          {t("organizationdetail.loadingOrganization")}
        </LoadingState>
      }
    >
      <Show
          ...
          <div class="space-y-8">
            <section class="rounded-2xl border border-line bg-surface p-6">
              <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p class="text-accent-label">
                    {org().type.replaceAll("_", " ")}
                  </p>
                  <h1 class="heading-strong-page mt-2">{org().name}</h1>
                  <p class="text-muted mt-3 max-w-3xl">{org().description}</p>
                </div>
                <a
                  href={`/organizations/${encodeURIComponent(org().slug)}/settings`}
