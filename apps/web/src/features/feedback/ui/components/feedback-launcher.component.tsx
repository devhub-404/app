...
import { Bug, ImagePlus, Lightbulb, MessageSquareWarning, Send, X } from 'lucide-solid';
...
import { createFeedbackFormSchema, type FeedbackFormInput } from '@/features/feedback/ui/schemas/forms.schema.ts';
...
function FeedbackLauncher() {
  ...
  const categories: Array<{ value: FeedbackCategory; label: string; icon: typeof Bug }> = [
    ...
    { value: 'issue', label: t('feedback.category.issue'), icon: MessageSquareWarning },
    { value: 'suggestion', label: t('feedbacklauncher.suggestion'), icon: Lightbulb },
  ];
  ...
  const [form, { Form, Field }] = createForm<FeedbackFormInput>({
    initialValues: { category: 'bug', description: '', contextUrl: '', screenshotMediaId: '' },
  ...
  const launch = () => {
    if (!auth.authenticated()) {
      redirectTo(`/login?redirect=${encodeURIComponent(getReturnToFromLocation())}`);
      return;
    }
  ...
  const submit = async (values: FeedbackFormInput) => {
    ...
    try {
      ...
      setResponse(form, { status: 'error', message: t('feedbacklauncher.sendFailedTryAgain') });
    }
  };
  ...
  return (
    <>
      <button
        type="button"
        onClick={launch}
        aria-label={t('feedbacklauncher.sendFeedback')} class="action action-primary fixed bottom-5 right-5 z-50 shadow-lg mobile:bottom-10"
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-3">
              ...
              <button type="button" onClick={() => close()} aria-label={t('feedbacklauncher.close')} class="action action-secondary">
                <X class="size-4" />
              </button>
            </div>
            <Field name="category">
              {() => (
                <div class="mt-5 grid grid-cols-3 gap-2">
                  {categories.map((item) => {
                    ...
                    return (
                      <button type="button" onClick={() => setValue(form, 'category', item.value)} class="action action-secondary">
                        <Icon class="size-4" />
                        {item.label}
                      </button>
                    );
            ...
            <div class="mt-4 grid gap-2">
              <label for="feedback-description" class="field-label">{t('feedbacklauncher.description')}</label>
              <Field name="description">
                {(field, fieldProps) => (
                  <>
                    ...
                    />
                    {field.error && (
                      <p class="text-danger-caption mt-1">
                        {field.error}
                      </p>
                    )}
                  </>
            ...
            <input
              ...
              onChange={async (event) => {
                ...
                try {
                  ...
                  if (result.error || !mediaId) throw new Error('SCREENSHOT_UPLOAD_FAILED');
                  setScreenshotMediaId(mediaId);
                } catch {
                  setResponse(form, { status: 'error', message: t('feedbacklauncher.couldNotAttachScreenshottryAgain') });
                } finally {
            ...
            <button
              type="button"
              aria-label={screenshotMediaId() ? t('feedback.screenshotAttached') : t('feedback.attachScreenshot')}
              disabled={attachingScreenshot() || form.submitting}
              onClick={() => screenshotInput.click()} class="action action-secondary"
            ...
            <button type="submit" disabled={form.submitting} aria-busy={form.submitting} class="action action-primary">
              <Send class="size-4" />
              {form.submitting ? t('feedback.sending') : t('feedbacklauncher.sendFeedback')}
            </button>
          </Form>
        </div>
      </Show>
